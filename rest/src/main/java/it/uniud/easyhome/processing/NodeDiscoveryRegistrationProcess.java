...
public class NodeDiscoveryRegistrationProcess extends Process {
    ...
    public NodeDiscoveryRegistrationProcess(int pid, UriInfo uriInfo,ProcessKind kind, LogLevel logLevel) throws NamingException, JMSException {
	...
	protected void process() throws JMSException, NamingException {
    	...
    	if (msg != null) {
        	...
        	if (NodeDiscoveryRspPacket.validates(pkt)) {
	        	log(LogLevel.FINE, "NodeDiscoveryRspPacket received from " + pkt.getSrcCoords());
	        	...
	        	try {
	        		...
	        		if (discPkt.getStatus() == ResponseStatus.SUCCESS) {
		        		...
		        		if (senderRetrievalResponse.getClientResponseStatus() == ClientResponse.Status.OK) {
		        			...
		        			log(LogLevel.FINE, "Sender node " + Node.nameFor(gatewayId, senderAddress) + " not found, hence discarding discovered node");
	        		}
