    	...
    	if (msg != null) {
    		...
    		if (event != null && event.getKind() == NetworkEvent.EventKind.NODE_ADDED) {
    	        ...
    	        try {
    	        	...
    	        	SimpleDescrReqPacket packet = new SimpleDescrReqPacket(node,++sequenceNumber);
    	            ObjectMessage outboundMessage = jmsSession.createObjectMessage(packet);
    	            getOutboundPacketsProducer().send(outboundMessage);    
    	            println("Simple descriptor request dispatched");
    	        } catch (Exception e) {
    	        	e.printStackTrace();
    	        	println("Simple descriptor request could not be dispatched to outbound packets topic");
    	        }
    		}
       	}
    }
